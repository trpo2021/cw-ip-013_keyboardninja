...
int main_menu(sf::RenderWindow& window, Difficult& difficult, std::vector <My_Sprite*> &static_spr_mas)
	...
	while (isMenu) {
		...
		if (!ShowDiff && !ShowScores) {
			if (IntRect(31, 19, 490, 60).contains(Mouse::getPosition(window))) {
				static_spr_mas[SPR_SCORE_MENU]->Get_sprite().setColor(Color::Red);
				select = 1;
			}
		...
		if (Mouse::isButtonPressed(Mouse::Left)) {
			if (!Select(select, ShowDiff, ShowRules, ShowScores)) return 0;
		}
